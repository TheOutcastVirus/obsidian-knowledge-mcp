...
export function updateFrontmatterInContent(content, updates) {
...
export function replaceWikilinkTarget(content, oldTarget, newTarget) {
    const links = extractWikilinks(content);
    let result = content;
    let offset = 0;
    for (const link of links) {
        if (link.target === oldTarget) {
            const newLink = link.raw.replace(`[[${oldTarget}`, `[[${newTarget}`);
...
export function insertWikilink(content, link, position) {
    const lines = content.split('\n');
...
export function findTextOccurrences(content, pattern, isRegex = false) {
    const occurrences = [];
    const lines = content.split('\n');
    const regex = isRegex
...
export function removeWikilink(content, target) {
    const links = extractWikilinks(content);
...
export function getLineContext(content, lineNumber) {
    const lines = content.split('\n');
